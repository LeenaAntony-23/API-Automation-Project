...
def test_fetch_one_customer_data_with_valid_customer_id(data, create_client, post_client_data,get_timeallocation_data_with_timeallocation_id ):
...
@pytest.mark.parametrize("data", ["./jsons/create_client.json"])
def test__fetch_one_customer_data_with_invalid_timeallocation_id(data, create_client, get_timeallocation_data_with_timeallocation_id):
    create_client = create_client(data, None, True)
    create_client_response = create_client.json()
    common.check_reponse_message(create_client_response, constants.add_client_success_message)
    logger.info("Client Details Added Successfully")
